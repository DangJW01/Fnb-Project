...
const OrderPage: React.FC<OrderPageProps> = ({ userId }) => {
  ...
  const fetchOrderData = async () => {
    ...
    try {
      ...
      const orderIdsResponse = await fetch(
        `http://backendfoodorder-prod.us-east-1.elasticbeanstalk.com/api/order`
      );
      if (!orderIdsResponse.ok) {
        console.error(
          `Failed to fetch order IDs. Status: ${orderIdsResponse.status}`
        );
        return;
      }
      ...
      const orderDetailsPromises = userOrderIds.map(async (orderId) => {
        ...
        if (!response.ok) {
          console.error(
            `Failed to fetch order details for order ${orderId}. Status: ${response.status}`
          );
          return null;
        }
        ...
        if (!orderStatusResponse.ok) {
          console.error(
            `Failed to fetch order status for order ${orderId}. Status: ${orderStatusResponse.status}`
          );
          return null;
        }
      ...
      const productDetailsPromises = flattenedOrderDetails.map((detail) =>
      ...
      productDetailsResults.forEach((data: any, index: number) => {
        setProductDetails((prevDetails) => ({
          ...prevDetails,
          [flattenedOrderDetails[index].productId]: data,
        }));
      });
    } catch (error) {
  ...
  const fetchProductDetails = async (productId: number) => {
    try {
      const response = await fetch(
        `http://backendfoodorder-prod.us-east-1.elasticbeanstalk.com/api/product/${productId}`
      );
      if (!response.ok) {
        console.error(
  ...
  const updateRating = async (orderId: number, rating: number) => {
    ...
    try {
      ...
      if (!orderDetailsResponse.ok) {
        console.error(
          `Failed to fetch order details for order ${orderId}. Status: ${orderDetailsResponse.status}`
        );
        return;
      }
      ...
      if (!updateResponse.ok) {
        console.error(
          `Failed to update rating. Status: ${updateResponse.status}`
        );
        return;
      }
    ...
    <ScrollView contentContainerStyle={styles.container}>
      <View style={styles.subContainer}>
        ...
        {orderIds.map((orderId) => {
          const orderStatus = orderDetails.find(
            (detail) => detail.orderId === orderId
          )?.orderStatus;
          const deliveryStatus = orderDetails.find(
            (detail) => detail.orderId === orderId
          )?.deliveryStatus;
          const ratings = orderDetails.find(
            (detail) => detail.orderId === orderId
          )?.ratings;
                ...
                .map((detail) => (
                  ...
                  >
                    <View>
                      ...
                      <Text>Price: RM{detail.price}</Text>
                      <Text>Total Amount: RM{detail.totalAmount}</Text>
                    </View>
                    <Image
                      source={{ uri: productDetails[detail.productId]?.image }}
              ...
              {orderStatus === "Completed" &&
                ...
                ratings === "Not given yet" && (
                  <>
                    ...
                    >
                      <Text
                        style={{ backgroundColor: "#F0E68C", color: "#fff" }}
                      >
                        RATE
                      </Text>
                    </TouchableOpacity>
...
const styles = StyleSheet.create({
  container: {
    padding: 20,
    backgroundColor: "#ddcbac",
  },
  subContainer: { height: "auto", backgroundColor: "#ddcbac" },
  ratingContainer: {
    flexDirection: "row",
