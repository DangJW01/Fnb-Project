...
const CartPage: React.FC<any> = ({ route }) => {
  ...
  const fetchCartDetails = async () => {
    try {
      const response = await fetch(
        `http://backendfoodorder-prod.us-east-1.elasticbeanstalk.com/api/cart`
      );
      if (!response.ok) {
        console.error(
      ...
      const userCartItems: CartItem[] = data.filter(
        (item: any) => item.userId === userId
      );
      setCartItems(userCartItems);

      // Fetch all product details
      const detailsPromises = userCartItems.map((item) =>
  ...
  const handleDelete = async (cartId: number) => {
    try {
      const response = await fetch(
        `http://backendfoodorder-prod.us-east-1.elasticbeanstalk.com/api/cart/${cartId}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleCheckout = async () => {
    try {
      const response = await fetch(
        `http://backendfoodorder-prod.us-east-1.elasticbeanstalk.com/api/order/${userId}/${totalAmount}`,
        {
          method: "POST",
          headers: {
          ...
          return (
            <View key={item.cartId} style={styles.cartItem}>
              ...
              <View style={styles.productDetails}>
                ...
                <Text style={styles.price}>Price: RM{productDetail.price}</Text>
                <Text style={styles.quantity}>Quantity: {item.quantity}</Text>
                <Text style={styles.totalAmount}>
                  Total Amount: RM{item.totalAmount}
                </Text>
              </View>
              <TouchableOpacity
              ...
              >
                <Icon name="trash-outline" size={25} color="#CD7F32" />
              </TouchableOpacity>
            </View>
          );
        })
