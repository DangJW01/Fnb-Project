...
const API_BASE_URL =
  "http://backendfoodorder-prod.us-east-1.elasticbeanstalk.com/api/category";
...
const ManageCategoryScreen: React.FC = () => {
  ...
  const handleUpdate = async () => {
    try {
      ...
      const currentCategoryResponse = await axios.get<CategoryData>(
      ...
      setCategories((prevCategories) => {
        return prevCategories.map((category) =>
          category.categoryId === categoryIdToUpdate
  ...
  return (
    <View style={styles.container}>
      <Modal isVisible={isModalVisible}>
        <View style={styles.modalContainer}>
          ...
          <View style={styles.modalButtonContainer}>
            <Button title="Cancel" color={"#baa27b"} onPress={toggleModal} />
            <Button title="Update" color={"#baa27b"} onPress={handleUpdate} />
          </View>
        </View>
      </Modal>
      ...
      >
        <Text style={{ marginRight: 37 }}>Name</Text>
        <TextInput
          style={styles.input}
          placeholder="Enter name"
      ...
      <View style={styles.addButtonContainer}>
        <Button color={"#baa27b"} title="Add Category" onPress={addCategory} />
      </View>

      <ScrollView style={styles.scrollView}>
        <View style={styles.categoryItem}>
          <Text style={[styles.tableHeader, { flex: 1 }]}>ID</Text>
          <Text style={[styles.tableHeader, { flex: 2 }]}>Name</Text>
          <Text style={[styles.tableHeader, { flex: 3 }]}>Description</Text>
          <Text style={[styles.tableHeader, { flex: 3 }]}>Actions</Text>
        </View>
        ...
        {categories.map((item) => (
          <View key={item.categoryId} style={styles.categoryItem}>
            ...
            <View style={styles.buttonContainer}>
              <TouchableOpacity
                style={styles.button}
                onPress={() => updateCategory(item.categoryId)}
              >
                <Text style={{ fontSize: 10, color: "#fff" }}>Update</Text>
              </TouchableOpacity>
