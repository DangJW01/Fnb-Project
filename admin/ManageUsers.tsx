...
const API_BASE_URL =
  "http://backendfoodorder-prod.us-east-1.elasticbeanstalk.com/api/user";
...
const ManageUsersScreen: React.FC = () => {
  ...
  const handleUpdate = async () => {
    try {
      ...
      const currentUserResponse = await axios.get<UserData>(
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <Text style={{ marginRight: 28 }}>Email</Text>
        <TextInput
          style={styles.input}
          placeholder="Enter email"
          value={newUserEmail}
      ...
      <View style={styles.addButtonContainer}>
        <Button color={"#baa27b"} title="Add User" onPress={addUser} />
      </View>
        ...
        {users.map((item) => (
          <View key={item.userId} style={styles.userItem}>
            ...
            <View style={styles.buttonContainer}>
              <TouchableOpacity
                style={styles.button}
                onPress={() => updateUser(item.userId)}
              >
                <Text>Update</Text>
              </TouchableOpacity>
