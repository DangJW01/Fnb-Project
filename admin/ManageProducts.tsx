...
const ManageProductScreen: React.FC = () => {
  ...
  const [isCategoryDialogVisible2, setIsCategoryDialogVisible2] =
    useState(false);
  ...
  const handleUpdate = async () => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/${selectedProduct.productId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
    ...
    <View>
      {categories.map((category) => (
        <TouchableOpacity
          ...
          onPress={() => setModalProductCategory(category.name)}
        >
  ...
  const renderCategoryRadioButtons2 = () => (
    <View>
      {categories.map((category) => (
        <TouchableOpacity
          key={category.categoryId}
          onPress={() => setNewProductCategory(category.name)}
        >
          <Text style={styles.categoryOption}>{category.name}</Text>
        </TouchableOpacity>
      ))}
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <View style={styles.categoryDialogContainer}>
          ...
          <Button
            color={"#baa27b"}
            title="Close"
            onPress={() => setIsCategoryDialogVisible2(false)}
          />
        </View>
      </Modal>
      ...
      <View style={{ marginBottom: 5 }}>
        <Button color={"#baa27b"} title="Add Product" onPress={addProduct} />
      </View>
